...
async def save_taqleed(event):
    ...
    if not reply:
        return await quick_edit(event, "❗ رد على رسالة لتقليدها.")
...
async def do_taqleed(event):
    ...
    try:
        if event.is_private:
            if saved_msg.text:
                await client.send_message(event.sender_id, saved_msg.text)
            if saved_msg.media:
                await client.send_file(event.sender_id, file=saved_msg.media)
        elif event.is_group:
            if saved_msg.text or saved_msg.voice or saved_msg.sticker:
                await event.reply(saved_msg.text or None, file=saved_msg.media if saved_msg.media else None)
...
async def save_ttl_media(event):
    ...
    try:
        ...
        caption = f"📥 وسائط مؤقتة من: {sender_name}"
        await client.send_file("me", file=event.media, caption=caption)
    except Exception as e:
...
async def notify_on_start():
    try:
        me = await client.get_me()
        await client.send_message("me", f"✅ البوت شغال أستاذ صعب، الحساب: @{me.username or me.first_name}")
    except: pass
